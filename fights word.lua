...
local AutoDeleteDropdown; local currentlyDisplayedEgg = nil;
...
-- New Intelligent Auto-Delete Updater
task.spawn(function()
    while task.wait(1) do
        local hatchGuis = lp.PlayerGui:FindFirstChild("HatchGuis")
        if not hatchGuis then continue end
        
        local visibleEggUI = nil
        for i, eggData in ipairs(ED) do
            local ui = hatchGuis:FindFirstChild(eggData.ID)
        ...
        if visibleEggUI then
            if currentlyDisplayedEgg ~= visibleEggUI.Name then
                currentlyDisplayedEgg = visibleEggUI.Name
                petNicknameToIdMap = {}; local options = {"None"};
                local petsFolder = visibleEggUI:FindFirstChild("Frame") and visibleEggUI.Frame:FindFirstChild("Pets")
                if petsFolder then
                    local petModels = {};
                    for _, petModel in ipairs(petsFolder:GetChildren()) do if petModel:IsA("Model") then table.insert(petModels, petModel) end end
                    table.sort(petModels, function(a, b) return a.Name < b.Name end);
                    for i, petModel in ipairs(petModels) do
                        local nickname = "Pet " .. i; table.insert(options, nickname); petNicknameToIdMap[nickname] = petModel.Name;
                    end
                end
                AutoDeleteDropdown:Refresh(options); AutoDeleteDropdown:Set({"None"});
            end
        elseif currentlyDisplayedEgg ~= nil then
            currentlyDisplayedEgg = nil
            AutoDeleteDropdown:Refresh({"None"}); AutoDeleteDropdown:Set({"None"});
        end
    end
end)
