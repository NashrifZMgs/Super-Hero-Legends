--[[
    Nexus-Lua Script (Version 36 - Autonomous Hunter)
    Master's Request: Implement the Controlled Behavioral Analysis module.
    Functionality: All features are now self-finding via a safe, one-time scan per session.
    Optimization: Fully automated remote finding, failsafe input blocking, and maximum resilience to game updates.
]]
...
--===================================================================--
-- This table stores the remotes once they are found, so the scan only runs once per session.
_G.FoundRemotes = {}
...
local BlockerFrame = Instance.new("Frame", InputBlocker)
BlockerFrame.Size = UDim2.new(1, 0, 1, 0)
BlockerFrame.BackgroundColor3 = Color3.new(0, 0, 0)
BlockerFrame.BackgroundTransparency = 0.5
BlockerFrame.Modal = true -- This property is the key. It absorbs all input.
local StatusLabel = Instance.new("TextLabel", BlockerFrame)
...
function Finder:ShowMessage(message)
...
-- The main scanning function. It takes a profile and finds the correct remote.
function Finder:ScanAndStore(profile)
    -- If we already found this remote, just start the feature.
    if _G.FoundRemotes[profile.CacheKey] then
        profile.Callback(_G.FoundRemotes[profile.CacheKey])
        return
    end

    self:ShowMessage("SCANNING: Acquiring " .. profile.Name .. " Remote...\nPlease wait and do not interact.")
    task.wait() -- Wait a frame for the UI to update
    ...
    if not success or not Services then
        self:ShowMessage("FAILURE: Could not find the game's Services folder.")
        task.wait(3); self:Hide()
        return
    end
    ...
    for _, service in ipairs(Services:GetChildren()) do
        ...
        if remoteFolder then
            for _, remote in ipairs(remoteFolder:GetChildren()) do
                ...
                -- Fire the remote with test arguments
                pcall(remote[profile.FireMethod], remote, unpack(profile.TestArgs))
                task.wait(0.2) -- Give the server a moment to respond
                
                if statObject.Value > baseline then
                    foundRemote = remote
                    break
                else
                    baseline = statObject.Value -- Reset baseline in case of random stat changes
                end
            end
        end
    ...
    if foundRemote then
        ...
        self:ShowMessage("FAILURE: Could not identify the " .. profile.Name .. " Remote.\nThe game may have updated significantly.")
        task.wait(3)
        self:Hide()
        Rayfield.Flags[profile.Flag]:Set(false)
    end
end
...
--                          SCRIPT FEATURES                          --
--===================================================================--
local Tabs = {ClicksTab, PetTab, UpgradesTab, MapTab, MiscTab, ProfileTab, SettingsTab} -- All tabs for easy reference
...
ClicksTab:CreateToggle({ Name = "Auto Click", CurrentValue = false, Flag = "AutoClickToggle", Callback = function(v)
    ...
    Finder:ScanAndStore({
        Name = "Auto Click", CacheKey = "ClickRemote", Flag = "AutoClickToggle",
        StatName = "\240\159\145\143 Clicks", RemoteType = "RE", FireMethod = "FireServer", TestArgs = {{}},
        Callback = function(remote)
            while _G.isAutoClicking do remote:FireServer({}); task.wait(0.05) end
        end
    })
end})
...
ClicksTab:CreateToggle({Name="Auto Rebirth",CurrentValue=false,Flag="AutoRebirthToggle",Callback=function(v)
    ...
    if not v then return end
    Finder:ScanAndStore({
        Name = "Auto Rebirth", CacheKey = "RebirthRemote", Flag = "AutoRebirthToggle",
        StatName = "\226\153\187\239\184\143 Rebirths", RemoteType = "RF", FireMethod = "InvokeServer", KnownName = "jag känner en bot, hon heter anna, anna heter hon", TestArgs = {1},
        Callback = function(remote)
            while _G.isAutoRebirthing do local id=table.find(rebirthOpts,RebirthDropdown.CurrentOption[1]);if id then pcall(remote.InvokeServer,remote,id)task.wait(0.5)end end
        end
    })
end})
...
PetTab:CreateToggle({Name="Auto Hatch Selected Egg (x3)",CurrentValue=false,Flag="AutoHatchToggle",Callback=function(v)
    _G.isAutoHatching=v
    if not v then AutoHatchStatusButton:Set("Status: Idle"); return end
    Finder:ScanAndStore({
        Name = "Auto Hatch", CacheKey = "HatchRemote", Flag = "AutoHatchToggle",
        StatName = "\240\159\165\154 Eggs", RemoteType = "RE", FireMethod = "FireServer", TestArgs = {"Basic", 1},
        Callback = function(remote)
            while _G.isAutoHatching do local sel=EggDropdown.CurrentOption[1];if sel and sel~="No Eggs Found"then AutoHatchStatusButton:Set("Status: Hatching "..sel);pcall(remote.FireServer,remote,sel,2)task.wait(0.05)else AutoHatchStatusButton:Set("Status: No egg selected");_G.isAutoHatching=false;Rayfield.Flags.AutoHatchToggle:Set(false);break end end;AutoHatchStatusButton:Set("Status: Idle")
        end
    })
end})
