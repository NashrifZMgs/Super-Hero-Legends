--[[
    Nexus-Lua Script (Version 25)
    Master's Request: Fix FireServer error and enable multi-select for Upgrade Farm.
    Functionality: UI Base, Live Stats, UI Control, Auto Click (Fixed), Auto Hatch, Auto Rebirth, Auto Upgrade, Multi-Select Farm
...
ClicksTab:CreateToggle({Name="Auto Rebirth",CurrentValue=false,Flag="AutoRebirthToggle",Callback=function(v) _G.isAutoRebirthing=v;if v then task.spawn(function() local s,rF=pcall(function()return game:GetService("ReplicatedStorage").Packages.Knit.Services:GetChildren()[REBIRTH_SERVICE_INDEX].RF["jag känner en bot, hon heter anna, anna heter hon"]end);if not s or not rF then Rayfield:Notify({Title="Error",Content="Rebirth remote needs updating.",Duration=7,Image="alert-triangle"});_G.isAutoRebirthing=false;Rayfield.Flags.AutoRebirthToggle:Set(false)return end;while _G.isAutoRebirthing do local id=table.find(rebirthOpts,RebirthDropdown.CurrentOption[1]);if id then pcall(rF.InvokeServer,rF,id)task.wait(0.5)end end end)end end})
...
UpgradesTab:CreateToggle({Name="Auto Upgrade Selected",CurrentValue=false,Flag="AutoUpgradeToggle",Callback=function(v) _G.isAutoUpgrading=v;if v then task.spawn(function() local s,uRF=pcall(function()return game:GetService("ReplicatedStorage").Packages.Knit.Services:GetChildren()[UPGRADE_SERVICE_INDEX].RF["jag känner en bot, hon heter anna, anna heter hon"]end);if not s or not uRF then Rayfield:Notify({Title="Error",Content="Upgrade remote needs updating.",Duration=7,Image="alert-triangle"});_G.isAutoUpgrading=false;Rayfield.Flags.AutoUpgradeToggle:Set(false)return end;while _G.isAutoUpgrading do if #UpgradeDropdown.CurrentOption>0 then for _,name in ipairs(UpgradeDropdown.CurrentOption)do local fmtName=string.lower(string.sub(name,1,1))..string.sub(name,2);pcall(uRF.InvokeServer,uRF,fmtName)task.wait(0.2);if not _G.isAutoUpgrading then break end end end;task.wait(0.5)end end)end end})

local UpgradeFarmSection=UpgradesTab:CreateSection("Upgrade Farm")
local FARM_SERVICE_INDEX,FARM_RF_INDEX=24,1
...
-- UPGRADED: Changed to multi-select
...
UpgradesTab:CreateToggle({Name="Auto Farm Selected",CurrentValue=false,Flag="AutoFarmToggle",Callback=function(v)
    ...
    if v then task.spawn(function()
        ...
        while _G.isAutoFarming do
            -- UPGRADED: Logic to handle multiple selections
            if #FarmDropdown.CurrentOption > 0 then
                for _, farmName in ipairs(FarmDropdown.CurrentOption) do
...
local PlaytimeButton,RebirthsButton,ClicksButton,EggsButton=ProfileTab:CreateButton({Name="Playtime: Loading...",Flag="PlaytimeStat",Callback=function()end}),ProfileTab:CreateButton({Name="Rebirths: Loading...",Flag="RebirthsStat",Callback=function()end}),ProfileTab:CreateButton({Name="Clicks: Loading...",Flag="ClicksStat",Callback=function()end}),ProfileTab:CreateButton({Name="Eggs: Loading...",Flag="EggsStat",Callback=function()end})
...
--============ LIVE DATA UPDATER ============--
spawn(function()local p=game:GetService("Players").LocalPlayer;local l=p:WaitForChild("leaderstats");local s=tick();while task.wait(1)do if not pcall(function()Rayfield:IsVisible()end)then break end;local el=tick()-s;PlaytimeButton:Set(string.format("Playtime: %02d:%02d:%02d",math.floor(el/3600),math.floor((el%3600)/60),math.floor(el%60)));local r=l:FindFirstChild("Rebirths");RebirthsButton:Set(r and"Rebirths: "..tostring(r.Value)or"Rebirths: N/A");local c=l:FindFirstChild("Clicks");ClicksButton:Set(c and"Clicks: "..tostring(c.Value)or"Clicks: N/A");local e=l:FindFirstChild("Eggs");EggsButton:Set(e and"Eggs: "..tostring(e.Value)or"Eggs: N/A")end end)
