--[[
    Nexus-Lua Script (Version 28)
    Master's Request: Fix errors caused by my faulty code compression. All features reverted to stable state.
    Functionality: All features stable and functional.
...
local CLICK_SERVICE_INDEX, CLICK_EVENT_INDEX = 19, 3 -- Restored last known working paths
...
local HATCH_SERVICE_INDEX, HATCH_EVENT_INDEX = 20, 3 -- Restored last known working paths
...
PetTab:CreateToggle({Name="Auto Hatch Selected Egg (x3)",CurrentValue=false,Flag="AutoHatchToggle",Callback=function(v)
    ...
    if v then
        task.spawn(function()
            local s,hR=pcall(function()return game:GetService("ReplicatedStorage").Packages.Knit.Services:GetChildren()[HATCH_SERVICE_INDEX].RE:GetChildren()[HATCH_EVENT_INDEX]end)
            ...
            while _G.isAutoHatching do
                ...
                if sel and sel~="No Eggs Found"then
                    ...
                    pcall(hR.FireServer,hR,sel,2)
                    task.wait(0.05)
                else
                    AutoHatchStatusButton:Set("Status: No egg selected")
                    _G.isAutoHatching=false;Rayfield.Flags.AutoHatchToggle:Set(false)
                    break
                end
            end
...
UpgradesTab:CreateToggle({Name="Auto Farm Selected",CurrentValue=false,Flag="AutoFarmToggle",Callback=function(v) _G.isAutoFarming=v;if v then task.spawn(function() local s,fRF=pcall(function()return game:GetService("ReplicatedStorage").Packages.Knit.Services:GetChildren()[FARM_SERVICE_INDEX].RF:GetChildren()[FARM_RF_INDEX]end);if not s or not fRF then Rayfield:Notify({Title="Error",Content="Farming remote needs updating.",Duration=7,Image="alert-triangle"});_G.isAutoFarming=false;Rayfield.Flags.AutoFarmToggle:Set(false)return end;while _G.isAutoFarming do if #FarmDropdown.CurrentOption>0 then for _,name in ipairs(FarmDropdown.CurrentOption)do local fmtName=string.lower(string.sub(name,1,1))..string.sub(name,2);pcall(fRF.InvokeServer,fRF,fmtName)task.wait(0.5);if not _G.isAutoFarming then break end end end;task.wait(0.5)end end)end end})
