--[[
    Nexus-Lua Script (Version 34 - Silent Failures)
    Master's Request: Remove remote error notifications and auto-disable on toggles for easier testing.
    Functionality: All features stable, with remote path failures now being silent.
    Optimization: Mobile/Touchscreen, Robust Loading, Uninterrupted Testing
]]
...
local CLICK_SERVICE_INDEX, CLICK_EVENT_INDEX = 1, 1
...
ClicksTab:CreateToggle({ Name = "Auto Click", CurrentValue = false, Flag = "AutoClickToggle", Callback = function(v)
    _G.isAutoClicking = v; if v then task.spawn(function()
        ...
        -- If remote is not found, fail silently without notification.
...
local REBIRTH_SERVICE_INDEX = 34
...
ClicksTab:CreateToggle({Name="Auto Rebirth",CurrentValue=false,Flag="AutoRebirthToggle",Callback=function(v) _G.isAutoRebirthing=v;if v then task.spawn(function()
    ...
    -- If remote is not found, fail silently without notification.
    if not s or not rF then return end
...
PetTab:CreateToggle({Name="Auto Hatch Selected Egg (x3)",CurrentValue=false,Flag="AutoHatchToggle",Callback=function(v) _G.isAutoHatching=v;if v then task.spawn(function()
    ...
    -- If remote is not found, fail silently without notification.
    if not s or not hR then return end
...
UpgradesTab:CreateToggle({Name="Auto Upgrade Selected",CurrentValue=false,Flag="AutoUpgradeToggle",Callback=function(v) _G.isAutoUpgrading=v;if v then task.spawn(function()
    ...
    -- If remote is not found, fail silently without notification.
    if not s or not uRF then return end
...
UpgradesTab:CreateToggle({Name="Auto Farm Selected",CurrentValue=false,Flag="AutoFarmToggle",Callback=function(v) _G.isAutoFarming=v;if v then task.spawn(function()
    ...
    -- If remote is not found, fail silently without notification.
    if not s or not fRF then return end
