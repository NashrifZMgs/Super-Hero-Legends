--[[
    Nexus-Lua Script (Version 31 - Stable Revert)
    Master's Request: Fix non-working features by removing the faulty Heuristic Scanner.
    Functionality: All features reverted to the stable, index-based remote finding method.
    Optimization: Mobile/Touchscreen, Robust Loading, Proven Logic
]]
...
local CLICK_SERVICE_INDEX, CLICK_EVENT_INDEX = 19, 3 -- [TODO: Update if needed]
...
local HATCH_SERVICE_INDEX, HATCH_EVENT_INDEX = 20, 3 -- [TODO: Update if needed]
...
PetTab:CreateToggle({Name="Auto Hatch Selected Egg (x3)",CurrentValue=false,Flag="AutoHatchToggle",Callback=function(v) _G.isAutoHatching=v;if v then task.spawn(function()
    ...
    while _G.isAutoHatching do local sel=EggDropdown.CurrentOption[1];if sel and sel~="No Eggs Found"then AutoHatchStatusButton:Set("Status: Hatching "..sel);pcall(hR.FireServer,hR,sel,2)task.wait(0.05)else AutoHatchStatusButton:Set("Status: No egg selected");_G.isAutoHatching=false;Rayfield.Flags.AutoHatchToggle:Set(false)break end end;AutoHatchStatusButton:Set("Status: Idle")
end)else AutoHatchStatusButton:Set("Status: Idle")end end})
...
local FARM_SERVICE_INDEX,FARM_RF_INDEX = 24, 1 -- [TODO: Update if needed]
