...
local Window = Rayfield:CreateWindow({
    ...
    ConfigurationSaving = {
        Enabled = true,
        FileName = "NexusFarmConfig_v34",
        UniqueIdentifier = "NexusFarmScript_034"
    }
})
...
local farmStates = {
    Strength = {isActive=false, thread=nil, selectedStation=1, flagName="StrFT_V34", teleportDelay=5},
    Speed    = {isActive=false, thread=nil, selectedStation=1, flagName="SpdFT_V34", teleportDelay=5},
    Aura     = {isActive=false, thread=nil, selectedStation=1, flagName="AurFT_V34", teleportDelay=5}
}
...
--[[ FARM TAB UI & LOGIC ]]
FarmTab:CreateSection("Auto Flight");FarmTab:CreateToggle({Name="Auto Flight Sequence",Default=autoFlightEnabledByUser,Flag="AutoFlightToggle_V34",Callback=function(v)autoFlightEnabledByUser=v;if autoFlightEnabledByUser then autoFlightActive=true;if autoFlightThread then task.cancel(autoFlightThread)end;print("Auto Flight ON.");autoFlightThread=task.spawn(function()while autoFlightEnabledByUser do if autoFlightActive then print("Flight Sequence Start.");pcall(function()initiateFlightRemote:InvokeServer()end);task.wait(1);pcall(function()chargeFlightRemote:FireServer(1)end);print("Flight Charged. Wait "..flightChargeDelay.."s.");task.wait(flightChargeDelay);if autoFlightEnabledByUser then print("Auto-restarting flight.");autoFlightActive=false;task.wait(0.3);autoFlightActive=true end else task.wait(0.1)end end;autoFlightThread=nil;autoFlightActive=false;print("Auto Flight master loop stopped.")end)else print("Auto Flight OFF.")end end});FarmTab:CreateInput({Name="Flight Charge Delay(s)",PlaceholderText="e.g.,6",Default=tostring(flightChargeDelay),Numeric=false,Flag="FlightChargeDelayInput_V34",Callback=function(T,FL)if FL then local nD=tonumber(T);if nD and nD>=0.1 and nD<=60 then flightChargeDelay=nD;print("Flight Delay:"..flightChargeDelay.."s")else print("Inv flight delay")end end end})

FarmTab:CreateSection("Global Station Farm Controls")
FarmTab:CreateDropdown({Name="Select Planet",Options=planetDisplayOptions,Default=defaultPlanetDisplay,Flag="PlanetSelectDropdown_V34",
    Callback=function(sV) -- SIMPLIFIED CALLBACK
        local sPS=getDropdownCallbackString(sV,"PlanetDd");if not sPS then return end
        if string.find(sPS,"Planet1")then selectedPlanetIdentifier="Planet1"
        elseif string.find(sPS,"Planet2")then selectedPlanetIdentifier="Planet2"
        elseif string.find(sPS,"Planet3")then selectedPlanetIdentifier="Planet3"
        end
        print("Selected Planet set to: "..selectedPlanetIdentifier)
    end
})
FarmTab:CreateToggle({Name="Enable Global Loop Delay",Default=useGlobalDelay,Flag="FarmDelayEnableToggle_V34",Callback=function(v)useGlobalDelay=v;print("Global Delay "..(v and"ON"or"OFF"))end})
FarmTab:CreateInput({Name="Global Delay(s)",PlaceholderText="e.g.,0.1",Default=tostring(globalDelayTime),Numeric=false,Flag="FarmDelayTimeInput_V34",Callback=function(T,FL)if FL then local nD=tonumber(T);if nD and nD>=0 and nD<=60 then globalDelayTime=math.max(0.1,nD);print("Global Delay:"..globalDelayTime.."s")else print("Inv delay")end end end})
FarmTab:CreateInput({Name="Global Burst Count",PlaceholderText="e.g.,10",Default=tostring(globalBurstCount),Numeric=false,Flag="FarmBurstCountInput_V34",Callback=function(T,FL)if FL then local nBC=tonumber(T);if nBC and nBC>=1 and nBC<=100 then globalBurstCount=math.floor(nBC);print("Global Burst:"..globalBurstCount)else print("Inv burst")end end end})
...
executeManagedFarmLoopWithPeriodicTeleport = function(statName, primaryRemote, preBurstActionFunc)
    local farmState = farmStates[statName]
    -- farmState.isActive is now set by the callback BEFORE this is called
    ...
    if farmState.isActive then
        ...
        farmState.thread = task.spawn(function()
            ...
            while farmState.isActive do
                ...
                local currentTeleportDelay = farmState.teleportDelay
            ...
            farmState.thread = nil
            if not farmState.isActive then print(statName .. " farming STOPPED.") end
        end)
    else 
        if farmState.thread then task.cancel(farmState.thread); farmState.thread = nil end
...
-- Farm Toggles with Correct Synchronization
FarmTab:CreateSection("Strength Training");FarmTab:CreateDropdown({Name="Station",Options=stationDropdownOptions,Default="Station 1",Flag="StrS_Dd_V34",Callback=function(v)local n=getStationNumFromDropdown(v,"StrDd");if n then farmStates.Strength.selectedStation=n;print("Sel StrS:"..n)end end});FarmTab:CreateInput({Name="TP Refresh (s)",PlaceholderText="e.g.,5",Default=tostring(farmStates.Strength.teleportDelay),Numeric=false,Flag="StrTPDelay_V34",Callback=function(T,FL)if FL then local nD=tonumber(T);if nD and nD>=1 and nD<=300 then farmStates.Strength.teleportDelay=nD;print("Str TP Delay:"..nD.."s")else print("Inv TP delay")end end end});FarmTab:CreateToggle({Name="Auto Farm Strength",Default=false,Flag=farmStates.Strength.flagName,Callback=function(value) farmStates.Strength.isActive=value; executeManagedFarmLoopWithPeriodicTeleport("Strength",strRemote,nil)end})
FarmTab:CreateSection("Speed Training");FarmTab:CreateDropdown({Name="Station",Options=stationDropdownOptions,Default="Station 1",Flag="SpdS_Dd_V34",Callback=function(v)local n=getStationNumFromDropdown(v,"SpdDd");if n then farmStates.Speed.selectedStation=n;print("Sel SpdS:"..n)end end});FarmTab:CreateInput({Name="TP Refresh (s)",PlaceholderText="e.g.,5",Default=tostring(farmStates.Speed.teleportDelay),Numeric=false,Flag="SpdTPDelay_V34",Callback=function(T,FL)if FL then local nD=tonumber(T);if nD and nD>=1 and nD<=300 then farmStates.Speed.teleportDelay=nD;print("Spd TP Delay:"..nD.."s")else print("Inv TP delay")end end end});FarmTab:CreateToggle({Name="Auto Farm Speed",Default=false,Flag=farmStates.Speed.flagName,Callback=function(value) farmStates.Speed.isActive=value; executeManagedFarmLoopWithPeriodicTeleport("Speed",spdRemote,activateSuperSpeedMode)end})
FarmTab:CreateSection("Aura Training");FarmTab:CreateDropdown({Name="Station",Options=stationDropdownOptions,Default="Station 1",Flag="AurS_Dd_V34",Callback=function(v)local n=getStationNumFromDropdown(v,"AurDd");if n then farmStates.Aura.selectedStation=n;print("Sel AurS:"..n)end end});FarmTab:CreateInput({Name="TP Refresh (s)",PlaceholderText="e.g.,5",Default=tostring(farmStates.Aura.teleportDelay),Numeric=false,Flag="AurTPDelay_V34",Callback=function(T,FL)if FL then local nD=tonumber(T);if nD and nD>=1 and nD<=300 then farmStates.Aura.teleportDelay=nD;print("Aur TP Delay:"..nD.."s")else print("Inv TP delay")end end end});FarmTab:CreateToggle({Name="Auto Farm Aura",Default=false,Flag=farmStates.Aura.flagName,Callback=function(value) farmStates.Aura.isActive=value; executeManagedFarmLoopWithPeriodicTeleport("Aura",aurEquilibriumRemote,beginAuraTraining)end})

--[[ OTHER TABS ]]
PetTab:CreateSection("Auto Hatch Eggs");PetTab:CreateDropdown({Name="Select Egg",Options=eggOptions,Default=defaultEggDisplay,Flag="EggS_Dd_V34",Callback=function(v)local s=getDropdownCallbackString(v,"EggDd");if s then selectedEggNameForHatch=getActualEggName(s);print("Sel Egg: "..selectedEggNameForHatch)end end});PetTab:CreateToggle({Name="Auto Hatch",Default=autoHatchingActive,Flag="EggHT_V34",Callback=function(v)autoHatchingActive=v;if autoHatchingActive then print("Hatch ON")task.spawn(function()while autoHatchingActive do for i=1,globalBurstCount do if not autoHatchingActive then break end pcall(function()eggPurchaseRemote:InvokeServer(selectedEggNameForHatch,1)end)end if useGlobalDelay and globalDelayTime>0 then task.wait(globalDelayTime)else task.wait()end end print("Hatch OFF")end)else print("Hatch OFF")end end})
PetTab:CreateSection("Pet Management");PetTab:CreateToggle({Name="Auto Equip Best",Default=autoEquipBestPetsActive,Flag="EqBPT_V34",Callback=function(v)autoEquipBestPetsActive=v;if autoEquipBestPetsActive then print("EquipBest ON (8s delay)")task.spawn(function()while autoEquipBestPetsActive do for i=1,globalBurstCount do if not autoEquipBestPetsActive then break end pcall(function()equipBestRemote:InvokeServer(true)end)end task.wait(8);end print("EquipBest OFF")end)else print("EquipBest OFF")end end})
PetTab:CreateToggle({Name="Auto Evolve All",Default=autoEvolvePetsActive,Flag="EvAPT_V34",Callback=function(v)autoEvolvePetsActive=v;if autoEvolvePetsActive then print("EvolveAll ON (8s delay)")task.spawn(function()while autoEvolvePetsActive do for i=1,globalBurstCount do if not autoEvolvePetsActive then break end pcall(function()evolveAllRemote:InvokeServer()end)end task.wait(8);end print("EvolveAll OFF")end)else print("EvolveAll OFF")end end})
MiscTab:CreateSection("General Automation");MiscTab:CreateToggle({Name="Auto Rebirth",Default=autoRebirthActive,Flag="RebT_V34",Callback=function(v)autoRebirthActive=v;if autoRebirthActive then print("Rebirth ON")task.spawn(function()while autoRebirthActive do for i=1,globalBurstCount do if not autoRebirthActive then break end pcall(function()rebirthRemote:InvokeServer()end)end if useGlobalDelay and globalDelayTime>0 then task.wait(globalDelayTime)else task.wait()end end print("Rebirth OFF")end)else print("Rebirth OFF")end end})
MiscTab:CreateToggle({Name="Auto Claim Quests",Default=autoClaimQuestsActive,Flag="ClQRT_V34",Callback=function(v)autoClaimQuestsActive=v;if autoClaimQuestsActive then print("ClaimQuests ON")task.spawn(function()while autoClaimQuestsActive do for i=1,globalBurstCount do if not autoClaimQuestsActive then break end pcall(function()claimQuestsRemote:InvokeServer()end)end if useGlobalDelay and globalDelayTime>0 then task.wait(globalDelayTime)else task.wait()end end print("ClaimQuests OFF")end)else print("ClaimQuests OFF")end end})
MiscTab:CreateToggle({Name="Auto Claim AFK Gifts",Default=autoClaimAFKGiftsActive,Flag="ClAFKT_V34",Callback=function(v)autoClaimAFKGiftsActive=v;if autoClaimAFKGiftsActive then print("ClaimAFK ON");currentAFKGiftToClaim=1;task.spawn(function()while autoClaimAFKGiftsActive do print("Claim AFK#"..currentAFKGiftToClaim)pcall(function()claimAFKGiftRemote:InvokeServer(currentAFKGiftToClaim)end)task.wait(2);currentAFKGiftToClaim=(currentAFKGiftToClaim%8)+1;if currentAFKGiftToClaim==1 then if useGlobalDelay and globalDelayTime>0 then print("Cycled AFK. GlobalDelay:"..globalDelayTime.."s");task.wait(globalDelayTime)else task.wait()end end end print("ClaimAFK OFF")end)else print("ClaimAFK OFF")end end})

Rayfield:LoadConfiguration()
print("Nexus-Lua Farm Bot V34 Loaded. Toggle state and timer logic corrected.")
