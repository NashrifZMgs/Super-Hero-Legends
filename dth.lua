...
FarmTab:CreateToggle({Name = "Enable Auto Train", CurrentValue = false, Flag = "AutoTrainToggle", Callback = function(Value)
    ...
    if autoTrainEnabled then
        ...
        autoTrainThread = task.spawn(function()
            ...
            if Worlds[selectedWorld] then TeleportRemote:InvokeServer(Worlds[selectedWorld]) end
            task.wait(2)
            while autoTrainEnabled do
                local strengthStat = LocalPlayer.leaderstats and LocalPlayer.leaderstats.Strength
                if strengthStat then
                    local playerStrength = ParseAbbreviatedNumber(strengthStat.Value)
                    local bestTreadmill = 1
                    local treadmillList = (selectedWorld == "Ocean World" and World2_Treadmills) or World1_Treadmills
                    ...
                    if bestTreadmill ~= currentTreadmillID then
                        Notify({Title="Auto Train", Content="Switching to Treadmill #" .. bestTreadmill, Duration=3, Image="trending-up"})
                        TreadmillRemote:FireServer(0)
                        local h = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid"); if h then task.wait(0.2);h.Jump=true;task.wait(0.5);h.Jump=true;task.wait(0.2) else task.wait(0.9) end
                    end
...
local destroyButton; local function resetDestroyButton() if destroyButton then destroyButton:Set("Destroy UI"); destroyButton.Callback = function() destroyButton:Set("Are you sure? Click again to confirm."); destroyButton.Callback = function() Rayfield:Destroy() end; task.delay(5,function() if destroyButton and destroyButton.Name == "Are you sure? Click again to confirm." then resetDestroyButton() end end) end end end
destroyButton=SettingTab:CreateButton({Name="Destroy UI",Callback=function()end})
resetDestroyButton()
-- ==============================================================================
--                                 FINALIZATION
-- ==============================================================================
