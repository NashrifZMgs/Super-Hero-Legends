...
local Window = Rayfield:CreateWindow({
    ...
    ConfigurationSaving = {
        Enabled = true,
        FileName = "NexusFarmConfig_v45_Final",
        UniqueIdentifier = "NexusFarmScript_045"
    }
})
...
local farmStates = {
    Strength = {isActive=false, thread=nil, flagName="StrFT_V45"},
    Speed    = {isActive=false, thread=nil, flagName="SpdFT_V45"},
    Aura     = {isActive=false, thread=nil, flagName="AurFT_V45"}
}

local autoFlightActive = false; local flightChargeDelay = 6; local autoFlightThread = nil
...
local TrainingR,PetSystemR,FlightR,HeroSelectionR=Remotes.Training,Remotes.Pets,Remotes.Flight,Remotes.HeroSelection
...
local selectHeroRemote = HeroSelectionR.SelectHero -- NEW HERO REMOTE

--[[ PLAYER STATS POINTERS ]]
...
local function beginAuraTraining(sO)if sO then pcall(function()aurBeginR:InvokeServer(sO)end);print("AuraTrainingBegin called.")else print("E:No station for AuraBegin")end end
...
ProfileTab:CreateSection("Hero Management");ProfileTab:CreateDropdown({Name="Switch Hero",Options={"Strength Hero","Aura Hero","Speed Hero"},Default="Strength Hero",Flag="SwitchHeroDropdown_V45",Callback=function(v)local s=getDropdownCallbackString(v,"HeroDd");if s then local hA=string.gsub(s," ","");print("Switching to: "..hA);pcall(function()selectHeroRemote:InvokeServer(hA)end)end end})

--[[ OTHER TABS ]]
PetTab:CreateSection("Auto Hatch Eggs");PetTab:CreateDropdown({Name="Select Egg",Options=eggOptions,Default=defaultEggDisplay,Flag="EggS_Dd_V45",Callback=function(v)local s=getDropdownCallbackString(v,"EggDd");if s then selectedEggNameForHatch=getActualEggName(s);print("Sel Egg: "..selectedEggNameForHatch)end end});PetTab:CreateToggle({Name="Auto Hatch",Default=autoHatchingActive,Flag="EggHT_V45",Callback=function(v)autoHatchingActive=v;if v then print("Hatch ON")task.spawn(function()while autoHatchingActive do for i=1,globalBurstCount do if not autoHatchingActive then break end pcall(function()eggPR:InvokeServer(selectedEggNameForHatch,1)end)end if useGlobalDelay and globalDelayTime>0 then task.wait(globalDelayTime)else task.wait()end end print("Hatch OFF")end)else print("Hatch OFF")end end})
PetTab:CreateSection("Pet Management");PetTab:CreateToggle({Name="Auto Equip Best",Default=autoEquipBestPetsActive,Flag="EqBPT_V45",Callback=function(v)autoEquipBestPetsActive=v;if v then print("EquipBest ON (8s delay)")task.spawn(function()while autoEquipBestPetsActive do for i=1,globalBurstCount do if not autoEquipBestPetsActive then break end pcall(function()eqBR:InvokeServer(true)end)end task.wait(8);end print("EquipBest OFF")end)else print("EquipBest OFF")end end})
PetTab:CreateToggle({Name="Auto Evolve All",Default=autoEvolvePetsActive,Flag="EvAPT_V45",Callback=function(v)autoEvolvePetsActive=v;if v then print("EvolveAll ON (8s delay)")task.spawn(function()while autoEvolvePetsActive do for i=1,globalBurstCount do if not autoEvolvePetsActive then break end pcall(function()evAR:InvokeServer()end)end task.wait(8);end print("EvolveAll OFF")end)else print("EvolveAll OFF")end end})
MiscTab:CreateSection("General Automation");MiscTab:CreateToggle({Name="Auto Rebirth",Default=autoRebirthActive,Flag="RebT_V45",Callback=function(v)autoRebirthActive=v;if v then print("Rebirth ON")task.spawn(function()while autoRebirthActive do for i=1,globalBurstCount do if not autoRebirthActive then break end pcall(function()rebR:InvokeServer()end)end if useGlobalDelay and globalDelayTime>0 then task.wait(globalDelayTime)else task.wait()end end print("Rebirth OFF")end)else print("Rebirth OFF")end end})
MiscTab:CreateToggle({Name="Auto Claim Quests",Default=autoClaimQuestsActive,Flag="ClQRT_V45",Callback=function(v)autoClaimQuestsActive=v;if v then print("ClaimQuests ON")task.spawn(function()while autoClaimQuestsActive do for i=1,globalBurstCount do if not autoClaimQuestsActive then break end pcall(function()clQR:InvokeServer()end)end if useGlobalDelay and globalDelayTime>0 then task.wait(globalDelayTime)else task.wait()end end print("ClaimQuests OFF")end)else print("ClaimQuests OFF")end end})
MiscTab:CreateToggle({Name="Auto Claim AFK Gifts",Default=autoClaimAFKGiftsActive,Flag="ClAFKT_V45",Callback=function(v)autoClaimAFKGiftsActive=v;if v then print("ClaimAFK ON");currentAFKGiftToClaim=1;task.spawn(function()while autoClaimAFKGiftsActive do print("Claim AFK#"..currentAFKGiftToClaim)pcall(function()clAFKR:InvokeServer(currentAFKGiftToClaim)end)task.wait(2);currentAFKGiftToClaim=(currentAFKGiftToClaim%8)+1;if currentAFKGiftToClaim==1 then if useGlobalDelay and globalDelayTime>0 then print("Cycled AFK. GlobalDelay:"..globalDelayTime.."s");task.wait(globalDelayTime)else task.wait()end end end print("ClaimAFK OFF")end)else print("ClaimAFK OFF")end end})

Rayfield:LoadConfiguration()
print("Nexus-Lua Farm Bot V45 Loaded. Hero Switcher added.")
